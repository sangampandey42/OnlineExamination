...
if (isset($_POST['username']) && $_POST['username'] && isset($_POST['email']) && $_POST['email'] && isset($_POST['pass']) && $_POST['pass'] && isset($_POST['typeofform']) && $_POST['typeofform'])
  {
    ...
    if ($typeajax == "studentform")
      {
        $stmt = $conn->prepare("SELECT s_email FROM studentdata WHERE s_email=?");
        $stmt->bind_param('s', $emailajax);
        $stmt->execute();
        $stmt->store_result();
        if ($stmt->num_rows == 0) //To check if the row exists
          {
            $stmt2 = $conn->prepare("INSERT INTO studentdata VALUES('',?,?,?)");
    ...
    elseif ($typeajax == "teacherform")
      {
        $stmt = $conn->prepare("SELECT t_email FROM teacherdata WHERE t_email=?");
        $stmt->bind_param('s', $emailajax);
        $stmt->execute();
        $stmt->store_result();
        if ($stmt->num_rows == 0) //To check if the row exists
          {
            $stmt2 = $conn->prepare("INSERT INTO teacherdata VALUES('',?,?,?)");
            $stmt2->bind_param('sss',$nameajax,$emailajax,$passajax);
            if($stmt2->execute()){
                echo json_encode(array(
                    'success' => 1,
    ...
    elseif ($typeajax == "adminform")
      {
        ...
        if ($stmt->num_rows == 0) //To check if the row exists
          {
            $stmt2 = $conn->prepare("INSERT INTO admindata VALUES('',?,?,?)");
            $stmt2->bind_param('sss',$nameajax,$emailajax,$passajax);
            if($stmt2->execute()){
                echo json_encode(array(
                    'success' => 1,
